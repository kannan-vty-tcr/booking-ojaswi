...
async function sendConfirmationEmail(email, phone, patientBookings, bookingId) {
  ...
  try {
    const html = _buildEmailHtml(email, phone, patientBookings, bookingId);
...
async function handleBook(req, res) {
  ...
  // Send confirmation email in the background — do NOT await so the success screen
  // appears immediately. Email errors are logged but never block the booking response.
  sendConfirmationEmail(email, phone, patientBookings, bookingId).catch(err => {
